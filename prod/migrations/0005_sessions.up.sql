...
-- sliding_expires_at is extended on each transparent IDP refresh, capped by
-- expires_at (created_at + 90 days hard cap).
--------------------------------------------------------------------------------
CREATE TABLE sessions (
    id                          TEXT PRIMARY KEY,           -- UUIDv7
    user_id                     TEXT NOT NULL,              -- FK → users.id
    token_hash                  TEXT NOT NULL UNIQUE,       -- SHA-256 of plaintext opaque token
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    sliding_expires_at          TIMESTAMPTZ NOT NULL,       -- Sliding: now + 7 days, capped at expires_at
    expires_at                  TIMESTAMPTZ NOT NULL,       -- Hard cap: created_at + 90 days, never updated
    ...
    oauth2_refresh_token        TEXT,                       -- NULL for providers without refresh tokens
    oauth2_token_expiry         TIMESTAMPTZ,               -- IDP access token expiry (~1hr typically)
    oauth2_id_token             TEXT,                       -- OIDC id_token; used as id_token_hint on logout
    oauth2_scope                TEXT,
    oauth2_token_type           TEXT NOT NULL DEFAULT 'Bearer',
    oauth2_token_refresh_count  INTEGER NOT NULL DEFAULT 0,
    oauth2_token_refresh_last_at TIMESTAMPTZ,
